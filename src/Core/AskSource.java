 ...
 *
 *          This state is prompts the user for the source of the money to be
 *          transfered in a cash account. As the header of the state suggests,
 *          it prompts the user to specify a source of a cash account.
 */
public class AskSource extends State {
	...
	void execute() {
		...
		if (in > 0) { // Conduct a transfer
			getContext().getTransClient().withdrawCash(portfolioMapping.get(in - 1), src, this.amount,
					getContext().getPort(), this.acct);
		} else { // Raw deposit
